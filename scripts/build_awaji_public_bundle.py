...
def _bundle_hotel_candidates(trip: dict, places: dict[str, dict[str, object]]) -> list[dict]:
    ...
    for candidate in _as_list(_as_dict(trip.get("candidate_sets")).get("hotels")):
        ...
        output.append({
            ...
            "distance_sources": [
                _public_provenance(candidate.get("primary_distance_source"))
            ] if isinstance(candidate.get("primary_distance_source"), dict) else [],
...
def build_public_bundle(trip: dict, trip_path: Path) -> dict:
    ...
    return {
        ...
        "presentation": {
            ...
            "preview_notice": _safe_str(_override_value(trip, "/presentation/preview_notice")),
        },
